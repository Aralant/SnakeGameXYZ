...
{
	//Сохраняем позицию позицию головы
	//Вставка новой части хвоста
...
void Snake::SetDirection(const EDirection newDirection)
{
	...
	// Если хвост змейки есть и есть коллизия между головой и началом хвоста при новом направлении движения, то выходим не меняя направление.
...
{
	UpdateBodyPositions();
	SetHeadPosition(_headPosition + _velocity); // Обновление позиции головы после обновления тела
}